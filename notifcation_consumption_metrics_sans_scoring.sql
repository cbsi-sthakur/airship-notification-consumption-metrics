  ...
  WITH hits AS (
    ...
    WHERE
     ...
     AND day_dt BETWEEN '2022-01-01' AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
    AND post_evar56_desc NOT LIKE '%gametracker%'
    -- AND LOWER(post_evar57_desc) LIKE '%comeback%kings%nfl%judge%'
  )
  , sportsapp AS (
    SELECT * FROM hits
...
WHERE notification_dt BETWEEN '2021-01-01' AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
-- AND LOWER(message_desc) LIKE '%comeback%kings%nfl%judge%'
-- AND message_desc LIKE '%Padres%'
)

  SELECT * FROM(
  -- EXCEPT(uniques, visits, page_views, video_plays, video_ad_plays, video_time_spent_mins, video_ad_time_spent_mins), SUM(uniques) AS uniques, SUM(visits) AS visits, SUM(page_views) AS page_views, SUM(video_ad_plays) AS video_ad_plays, SUM(video_time_spent_mins) AS video_time_spent_mins, SUM(video_ad_time_spent_mins) AS video_ad_time_spent_mins  FROM(
  -- Total
  SELECT
    notification_dt as ds,
  ...
  -- ORDER BY 1 DESC, direct_open_cnt DESC,1,2,3,4,5
